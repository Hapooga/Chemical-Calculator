...
unique_ptr<Expression<float>> Parse(string_view& formula, const PeriodicTable& periodic_table) {
	...
	if (formula.front() == '[' || formula.front() == ']' ||
		...
		return Parse(formula, periodic_table);
	}
	// =========================================================================================================================
	if (formula.front() == '(') {
		...
		unique_ptr<Expression<float>> sub_formula = Parse(GetSubFormula(formula), periodic_table);
		...
		// -----------------------------------
		return Sum(Product(Value(sub_formula->Evaluate()), Value(l_value)), Parse(formula, periodic_table));
	}
	// =========================================================================================================================
	if (isdigit(formula.front())) {
		...
		if (isupper(formula.front())) {
			...
			return Product(Value(l_value), Parse(formula, periodic_table));
		}
		// -----------------------------------
	}
	// =========================================================================================================================
	if (isupper(formula.front())) {
		...
		float l_value = periodic_table.GetAtomicWeightFromClass((GetElement(formula)));
		// -----------------------------------
		if (formula.empty()) {
		...
		if (isupper(formula.front())) {
			...
			return Sum(Value(l_value), Parse(formula, periodic_table));
		}
		// -----------------------------------
		if (isdigit(formula.front())) {
			...
			return Sum(Product(Value(l_value), Value(r_value)), Parse(formula, periodic_table));
		...
		if (formula.front() == '(') {
			...
			// ------------------
			return Sum(Sum(Value(l_value), Product(Value(sub_formula->Evaluate()), Value(r_value))), Parse(formula, periodic_table));
		}
		// -----------------------------------
		return Parse(formula, periodic_table);
	}
...
String^ ConvertToMolecularWeight(String^ сhemical_formula) {
	...
	string formula = marshal_as<string>(сhemical_formula);

	CheckFormulaAndThrowException(formula);

	unique_ptr<Expression<float>> formula_expression;

    const static PeriodicTable periodic_table;

	formula_expression = Parse(string_view(formula), periodic_table);
